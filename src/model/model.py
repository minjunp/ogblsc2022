...
from model.tensor_models import logsigmoid
from model.tensor_models import abs
from model.tensor_models import masked_select
from model.tensor_models import get_device, get_dev
from model.tensor_models import norm
from model.tensor_models import get_scalar
from model.tensor_models import reshape
from model.tensor_models import cuda
from model.tensor_models import ExternalEmbedding
from model.tensor_models import InferEmbedding
from model.score_fun import *
from model.loss import LossGenerator
...
class KEModel(object):
    ...
    def load_emb(self, path, dataset):
        ...
        """
        self.entity_emb.load(path, 'entity_feat')
        self.relation_emb.load(path, 'relation_feat')
    ...
    def forward_test_wikikg(self, query, ans, candidate, mode, logs, gpu_id=-1):
        ...
        """
        scores = self.predict_score_wikikg(query, candidate, gpu_id=gpu_id)
        batch_size = query.shape[0]
        top10s = []
   
        for i in tqdm(range(batch_size)):
            score = scores[i].tolist()
            cand = candidate[i].tolist()
            pairs = list(zip(cand, score))
            pairs2 = sorted(pairs, key=lambda x: x[1], reverse=True)
            
            top10 = [p[0] for p in pairs2[0:10]]
            top10s.append(top10)

        print('forward_test_wikikg in process...')
        if mode == "valid":
            neg_scores = reshape(scores, batch_size, -1)
            for i in tqdm(range(batch_size)):
                # Check if ans[i] is in the list, else put the ranking to the last
                if ans[i] not in candidate[i, :]:
                    ranking = 1000
                else:
                    # Get the index of answer in candidate
                    ind = candidate[i, :].tolist().index(ans[i])
                    ranking = F.asnumpy(F.sum(neg_scores[i] >= neg_scores[i][ind], dim=0) + 1)
    ...
    def predict_score_wikikg(self, query, candidate, gpu_id=-1):
        num_chunks = len(query) # number of triplets (16)
        chunk_size = 1
